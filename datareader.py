...
class YcbineoatReader:
  ...
  def get_upscale_rgb(self,i):
    color = imageio.imread(self.color_files[i].replace('rgb','upscale/upscayl_png_realesrgan-x4plus_3x'))[...,:3]
    return color
...
class ClosePoseReader(YcbineoatReader):
  def __init__(self,video_dir,object_id=55, downscale=1, shorter_side=None, zfar=np.inf):
    self.video_dir = video_dir
    self.downscale = downscale
    self.zfar = zfar
    # Get all the PNG files matching the pattern 'xxxxx-color.png' in the video_dir
    self.color_files = sorted(glob.glob(os.path.join(self.video_dir, '*-color.png')))
    
    # Ensure that color_files isn't empty before proceeding
    if not self.color_files:
        raise ValueError(f"No color files found in directory: {self.video_dir}")
    self.metadata= scipy.io.loadmat(os.path.join(self.video_dir, 'metadata.mat'))
    if DEBUG:
      print(len(self.metadata["000000"]))
    self.obj_ids= sorted([int(s[0]) for s in self.metadata["000000"][0][0][0]])
    self.K= self.metadata["000000"][0][0][3]

    self.object_id=object_id
    self.id_str = [ s for s in self.metadata.keys() if s.isdigit()]
    self.H,self.W = cv2.imread(self.color_files[0]).shape[:2]
    if shorter_side is not None:
      self.downscale = shorter_side/min(self.H, self.W)
    self.H = int(self.H*self.downscale)
  ...
  def get_mask(self,i):
    object_to_track=self.object_id
    mask_file_name=f"{i:06d}-label.png"
    mask=cv2.imread(os.path.join(self.video_dir,mask_file_name),-1)
    
    if DEBUG:
      from matplotlib import colors as mcolors
      import matplotlib.patches as mpatches
      mask =mask.astype(np.uint8)
      # Get the unique object IDs in the scene, excluding 0 (background)
      unique_ids = np.unique(mask)
      unique_ids = unique_ids[unique_ids != 0]  # Ignore background (0)

      # Create a dictionary to assign a unique color to each object ID
      cmap = plt.get_cmap('hsv', len(unique_ids))  # Use 'hsv' for distinct colors
      color_dict = {obj_id: cmap(i) for i, obj_id in enumerate(unique_ids)}

      # Create a new RGB image to hold the colorized mask
      colorized_mask = np.zeros((*mask.shape, 3), dtype=np.float32)

      # Apply the color to each object ID in the mask
      for obj_id in unique_ids:
          colorized_mask[mask == obj_id] = color_dict[obj_id][:3]  # Assign RGB color

      # Plot the colorized mask
      plt.figure(figsize=(10, 10))
      plt.imshow(colorized_mask)

      # Overlay the object IDs at the centroid of each object
      for obj_id in unique_ids:
        positions = np.argwhere(mask == obj_id)  # Find all pixels of the current object
        if len(positions) > 0:
          centroid = positions.mean(axis=0)  # Calculate the centroid (average position)
          plt.text(centroid[1], centroid[0], str(obj_id), color='black', fontsize=12, 
                    ha='center', va='center', bbox=dict(facecolor='white', edgecolor='none', pad=2))

      plt.title("Multi-Level Mask Visualization (Distinct Colors for Present Objects)")
      plt.savefig("mask.png")
      plt.close()
    mask[mask!=object_to_track]=0
    mask[mask==object_to_track]=255
    if DEBUG:
     
      cv2.imwrite("mask_object.png",mask)
      # plt.savefig("mask_object.png")
    return mask
...
if __name__=="__main__":
  reader=ClosePoseReader("/mnt/ssd_990/teng/ycb/FoundationPose/demo_data/closepose/set8/scene5/", object_id=55)
  print(reader.K, "intrinsic matrix")
  reader.get_depth(0)
  reader.get_mask(0)
  reader.get_color(0)
