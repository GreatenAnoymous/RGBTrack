...
class YcbineoatReader:
    ...
    def get_upscale_rgb(self, i):
        color = imageio.imread(
            self.color_files[i].replace(
                "rgb", "upscale/upscayl_png_realesrgan-x4plus_3x"
            )
        )[..., :3]
        return color
    ...
    def __init__(self, base_dir, zfar=np.inf, resize=1):
        ...
        if os.path.exists(f"{self.base_dir}/scene_gt.json"):
            ...
            assert len(self.scene_gt) == len(self.color_files), (
                f"scene_gt.json has {len(self.scene_gt)} entries, "
                f"but there are {len(self.color_files)} frames"
            )
        else:
            self.scene_gt = None
...
class ClosePoseReader(YcbineoatReader):
    def __init__(
        self, video_dir, object_id=55, downscale=1, shorter_side=None, zfar=np.inf
    ):
        self.video_dir = video_dir
        self.downscale = downscale
        self.zfar = zfar
        ...
        self.color_files = sorted(
            glob.glob(os.path.join(self.video_dir, "*-color.png"))
        )
    ...
    def get_mask(self, i):
        ...
        if DEBUG:
            ...
            for obj_id in unique_ids:
                colorized_mask[mask == obj_id] = color_dict[obj_id][
                    :3
                ]  # Assign RGB color
            ...
            for obj_id in unique_ids:
                positions = np.argwhere(
                    mask == obj_id
                )  # Find all pixels of the current object
                if len(positions) > 0:
            ...
            plt.title(
                "Multi-Level Mask Visualization (Distinct Colors for Present Objects)"
            )
