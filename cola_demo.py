...
if __name__ == "__main__":
    ...
    for i in range(len(reader.color_files)):
        ...
        if i == 0:
            ...
            # )
            pose= binary_search_depth(est,mesh, color, mask, reader.K,w=544,h=960, depth_min=0.2, depth_max=6,debug=False)
            # pose = est.register(
            #     K=reader.K,
            #     rgb=color,
            #     depth=initial_depth,
            ...
            if SAVE_VIDEO:
                output_video_path = "cola_can.mp4"  # Specify the output video filename
